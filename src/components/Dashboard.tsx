...
import { Utensils, Droplets, Footprints, AlertCircle, History, Filter, Clock, TrendingUp, Calendar, Zap, Crown } from 'lucide-react';
    ...
    <div className="w-full space-y-12 animate-in fade-in duration-1000 overflow-visible">
      ...
      <div className="relative pt-6 flex flex-col items-center">
        ...
        <div className="relative group">
          ...
          <div className="relative w-44 h-44 rounded-full border-[6px] border-white p-1 overflow-hidden shadow-4xl bg-white transition-all duration-700 group-hover:scale-105">
        ...
        <div className="mt-10 text-center px-6">
          ...
          <div className={`mt-10 glass px-10 py-6 rounded-[3rem] max-w-sm mx-auto shadow-2xl transition-all duration-1000 ${isAvatarAnimating ? 'scale-105 bg-white' : 'scale-100'}`}>
      ...
      <div className="px-2 space-y-4">
        <div className="flex items-center justify-between px-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[11px] font-black uppercase tracking-[0.4em] text-luxe-deep opacity-30">Vitals</h2>
          </div>
          <TrendingUp size={16} className="text-luxe-deep opacity-10" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="grid grid-rows-2 gap-4">
            <div className="card-pearl p-6 flex items-center gap-5">
              <div className="w-14 h-14 rounded-3xl bg-blue-50 flex items-center justify-center text-blue-500 shadow-inner">
                <Droplets size={24} />
              </div>
              <div>
            ...
            <div className="card-pearl p-6 flex items-center gap-5">
              <div className="w-14 h-14 rounded-3xl bg-amber-50 flex items-center justify-center text-luxe-gold shadow-inner">
                <Zap size={24} />
              </div>
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-luxe-deep/20">Energy</span>
                <p className="text-2xl font-bold text-luxe-deep tracking-tight">{Math.round(stats.energy * 100)}<span className="text-sm opacity-20 font-medium ml-0.5">%</span></p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-8 transition-all duration-700 ${stats.urgency > 0.7 ? 'scale-[1.03]' : 'scale-100'}`}>
          <div className={`card-pearl p-10 relative overflow-hidden transition-all duration-700 ${stats.urgency > 0.8 ? 'border-luxe-orange/30 shadow-[0_30px_70px_rgba(255,140,0,0.1)]' : ''}`}>
            ...
            <div className="flex items-center justify-between">
              ...
              {stats.urgency > 0.7 && (
                <button className="px-6 py-3 bg-luxe-deep text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-transform">
                  Take Break
                </button>
              )}
            </div>
      ...
      <div className="px-4 space-y-8">
        <div className="flex items-center justify-between px-4">
          ...
          <button className="p-4 glass rounded-[1.5rem] text-luxe-deep opacity-30 hover:opacity-100 transition-all hover:bg-white shadow-lg">
            <Filter size={20} />
          </button>
        </div>
          ...
          {filterOptions.map((opt) => (
            <button
              ...
              className={`flex-shrink-0 px-8 py-4 rounded-[2rem] text-[11px] font-black uppercase tracking-[0.3em] transition-all duration-500 border-2 ${filterType === opt.value
                  ? 'bg-luxe-deep text-white border-luxe-deep shadow-2xl scale-105'
                  : 'bg-white border-white text-luxe-deep/30 hover:text-luxe-deep/80 hover:shadow-lg'
                }`}
            >
              {opt.label}
            </button>
          ...
          {filteredEvents.length === 0 ? (
            ...
            filteredEvents.map((event) => (
              ...
              >
                ...
                <div className="card-pearl p-8 flex items-center justify-between group-hover:bg-luxe-base transition-all duration-500">
                  <div className="flex items-center gap-6">
                    <div className={`w-16 h-16 rounded-[2rem] flex items-center justify-center bg-luxe-base transition-all duration-1000 group-hover:scale-110 shadow-inner group-hover:rotate-6 ${getEventColor(event.type)}`}>
